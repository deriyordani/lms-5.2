...
Class Auth extends CI_Controller{
	...
	function generate_pass(){
		echo password_hash('adminlms2021!', PASSWORD_BCRYPT);
	}
			...
			if ($query->num_rows() > 0) {
				...
				if(password_verify($password,$row->password)){
					...
					if ($row->is_active == 1) {
						...
						$this->user_m->update_data(array('last_login' => time_format(current_time(),'Y-m-d H:i')), array('uc' => $row->uc, ));
	...
	function store_register(){
		if ($this->input->post('f_save')) {
			...
			if ($type_person == 2) {
				...
				$this->instructor_m->insert_data($data_ins);
			}
