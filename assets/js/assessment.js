//var base_url = 'http://192.168.1.17:9999/lms-ng-rev4/';
...
function save_answer(){
	...
	if (q_type == 3) {
		var uc_assque 	= $('input[name=f_uc_assque]').val();
		var answer 		= tinyMCE.get('txarea-'+uc_assque).getContent();
		var uc_answer	= $('input[name=f_uc_ess_answer]').val();
		var uc_attempt 	= $('input[name=f_att_code]').val();

		//alert(answer+' | '+uc_attempt+' | '+uc_assque);

		$.ajax({
				type		: 'post',
				dataType	: 'json',
				data 		: {js_uc_attempt : uc_attempt, js_uc_assque : uc_assque, js_answer : answer, js_uc_answer : uc_answer, js_time_running : time_running},
				url			: base_url+'assessment/save_essay_answer_ajax',
